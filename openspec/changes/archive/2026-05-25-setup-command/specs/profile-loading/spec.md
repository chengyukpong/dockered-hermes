...
The system SHALL merge `defaults` and the named profile. The `env` and `config` fields SHALL be deep-merged with profile values overriding defaults on conflict. The `soul` field SHALL be replaced entirely by the profile value if present.

#### Scenario: Profile exists and merges correctly
- **WHEN** a profile name exists in the `profiles` section
- **THEN** the system returns a merged config where `env` and `config` are deep-merged and `soul` is the profile value (or default if not specified)
