...
The system SHALL check that a container named `hermes-<name>` exists using `podman inspect` before attempting to start or exec. If no such container exists, the system SHALL throw an error.
...
The system SHALL execute `podman exec -it hermes-<name> /opt/hermes/.venv/bin/hermes` with stdio inherited from the parent process, providing an interactive TUI session. The full path to the hermes binary inside the container is `/opt/hermes/.venv/bin/hermes`.

#### Scenario: Successful interactive session
- **WHEN** the container is running and the user's terminal is interactive
- **THEN** the system spawns `podman exec -it hermes-<name> /opt/hermes/.venv/bin/hermes` with `{ stdio: 'inherit' }` and waits for the ChildProcess to exit
